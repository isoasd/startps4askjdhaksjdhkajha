...
bot.on("ready", async () => {
  ...
  bot.user.setActivity("Fortnite PS4", {type: "PLAYING"});
});
...
bot.on("message", async message => {
  ...
  if(commandfile) commandfile.run(bot,message,args);
 	if(message.channel.id === "496203607419387904"){
  	if(message.content === "!startpc" && message.member.roles.find(r => r.name === "Scrim Staff PS4")){
	  ...
	  let last3chan = message.guild.channels.find(c => c.name === "scrim-last3-ps4")
	  if(message.content === "!cls" && message.member.roles.find(r => r.name === "Scrim Staff PS4")){
	...
	let last3ch = message.guild.channels.find("name", "scrim-last3-ps4");
...
// 		  	});
		  ...
		  try{
			...
			let last3chan = message.guild.channels.find(`name`, "scrim-last3-ps4");
